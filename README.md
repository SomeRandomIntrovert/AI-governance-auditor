...
| Autonomy tier | Observe / Advise / Act with Approval / Act Autonomously — verified via forward reachability from every trigger, treating gate nodes as dead ends, not presence-checking | Adapted from Gartner's AI-agent autonomy model (May 2026) |
| Audit trail | Presence of logging/history-capturing nodes | Governance best practice |

## Findings — 68 real, public n8n workflows

Autonomy classification checks whether a write action is reachable from
every trigger only via a path that passes through a verified gate (not
presence-anywhere checking, and not merely "some path has a gate" —
a single ungated path, such as a bypassed fallback branch, disqualifies
the whole action), plus operation-aware write-action detection (not
just node type). See Methodology below.

| Metric | Value |
|---|---|
| Workflows audited | 68 |
| Average composite risk score | 57.72 / 100 |
| Act fully autonomously, no verified approval gate | **55.88%** |
| Genuinely gated ("Act with Approval") | 2.94% |
| Have structured error handling | **0.00%** |
| Have any audit-trail logging | 7.35% |
| Act Autonomously **and** no error handling **and** no logging | **51.47%** |
...
2. **A write action must be unreachable from every trigger without
   passing through a verified gate.** Checked via forward reachability:
   starting from every trigger node, walk the workflow's `connections`
   graph forward, treating gate nodes as dead ends. If the write action
   is still reachable, at least one path bypasses every gate — this
   correctly catches a real failure pattern where a gate is wired
   correctly on the primary path but a retry or fallback branch bypasses
   it entirely, which a weaker "does some gated path exist" check would
   miss. A single ungated path anywhere drops the whole workflow to "Act
   Autonomously," matching Gartner's own definition that this tier
   requires *every* action to be gated.
...
misidentified as hardcoded secrets), manually confirmed the "Act
Autonomously" flag against the Discord bot example above via verified
graph traversal, and validated scoring direction with a before/after
pair on deliberately hardened vs. unhardened versions of the same
workflow logic:

| | Composite Score | Autonomy Tier |
|---|---|---|
| Before hardening | 37.8 | Act Autonomously |
| After hardening | 84.2 | Act with Approval |

Known limitations documented honestly in `docs/validation_notes.md` —
including where the current heuristics can still be wrong (node-type
detection completeness, audit-trail keyword matching) — along with a
full record of every real bug found and fixed during development,
several via public review after the initial launch: a presence-only gate
check that didn't verify position, a trigger node being miscounted as a
mid-flow checkpoint, read operations being miscounted as writes, and a
"some path is gated" check that missed a fallback branch bypassing the
gate entirely. Each fix's measured before/after impact on the 68-workflow
corpus is reported in full — including the fixes that changed nothing
measurable, not just the ones that moved the headline numbers.
