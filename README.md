...
| Average composite risk score | 63.34 / 100 |
| Fully autonomous (write actions, zero approval gate) | **27.94%** |
| Have structured error handling | **0.00%** |
| Have any audit-trail logging | 7.35% |
| Autonomous **and** no error handling **and** no logging | **25.00%** |
| Highest-risk category | Notion (avg. 46.27) |

The compound stat is the sharpest finding: a quarter of these real,
publicly-used AI agent templates can take multiple real-world actions
with zero human approval, fail silently if something breaks, and leave
no record either way if something goes wrong.
